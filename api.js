...
app.post('/api/register/', async (req, res, next) =>
{
  ...
  console.log("regi_data",FirstName, LastName , Login , Password , Email);
...
app.post('/api/forgotpassword/', async (req, res, next) =>
{
  ...
  // console.log("wwwwwwwwwwwww", Email);
...
app.post('/api/test/', auth, async (req, res, next) => 
{
  console.log("~~~~~~~~~~~~~~~~~~~");
  res.send("jwt_test");
});
...
app.post('/api/login/', async (req, res, next) => 
{
  ...
  const accessToken = signToken(login);
  if(!login){
  ...
  passport.authenticate('local', {session: false}, function(err, user, info){
   ...
   //   return res.json({user: user.toAuthJSON()});
        var ret = {id: user.id, firstName: user.Firstname, lastName: user.Lastname, error: '', accessToken: accessToken};
